...
class DepartamentoForm(forms.ModelForm):
    cargos = forms.ModelMultipleChoiceField(
        queryset=Cargo.objects, required=False, label="cargos")
    empleados = forms.ModelMultipleChoiceField(
        queryset=Empleado.objects, required=False, label="empleados")

    def __init__(self, *args, **kwargs):
        super(DepartamentoForm, self).__init__(*args, **kwargs)
        if self.instance.pk:
            self.fields['cargos'].queryset = Cargo.objects.filter(
                departamento=self.instance)
            self.fields['empleados'].queryset = Empleado.objects.filter(
                cargo__departamento=self.instance)
        # end if
    # end def
...
class NominaForm(forms.ModelForm):
    ...
    def save(self, commit):
        ...
        ultima_nomina = LiquidacionNomina.objects.filter(
            fecha_corte__lt=obj.fecha_corte, empleado=obj.empleado, contrato=contrato).last()
        primera_nomina = LiquidacionNomina.objects.filter(
            fecha_corte__lt=obj.fecha_corte, empleado=obj.empleado, semestre=obj.semestre, fecha_corte__year=today.year).first()
        ...
        hexd = HorasExtra.objects.filter(dominical=False, tipo=True, empleado=obj.empleado,
                                         semestre=obj.semestre, contrato=contrato).aggregate(total=Sum(F('horas')))['total'] or 0
        hexn = HorasExtra.objects.filter(dominical=False, tipo=False, empleado=obj.empleado,
                                         semestre=obj.semestre, contrato=contrato).aggregate(total=Sum(F('horas')))['total'] or 0
        hxdd = HorasExtra.objects.filter(dominical=True, tipo=True, empleado=obj.empleado,
                                         semestre=obj.semestre, contrato=contrato).aggregate(total=Sum(F('horas')))['total'] or 0
        hxnd = HorasExtra.objects.filter(dominical=True, tipo=False, empleado=obj.empleado,
                                         semestre=obj.semestre, contrato=contrato).aggregate(total=Sum(F('horas')))['total'] or 0
        ...
        corte_anual = date(obj.fecha_corte.year - 1,
                           conf.mes_canu, conf.dia_canu)
        ...
        valor_hora = (salario / [15, 30]
                      [contrato.modalidad]) / contrato.horas_dia
        ...
        if fecha_anterior.day >= conf.dia_cqui:
            este_corte = date(obj.fecha_corte.year,
                              obj.fecha_corte.month, conf.dia_cmes)
        elif fecha_anterior.day >= conf.dia_cmes:
            este_corte = date(obj.fecha_corte.year,
                              obj.fecha_corte.month, conf.dia_cmes)
        # end if
        dias_propios = (este_corte - fecha_anterior).days
        pago_ordinario = fecha_anterior.day == [
            conf.dia_cqui, conf.dia_cmes][contrato.modalidad]
